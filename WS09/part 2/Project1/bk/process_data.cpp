...
namespace sdds_ws9 {
	...
	ProcessData::ProcessData(std::string filename, int n_threads) {  
		...
		if (ifstr) {
			ifstr.read((char*)&doc_size, sizeof(int));
		}
		...
		int index = 0;
		while (ifstr.good()) {
			ifstr.read((char*)&(data[index]), sizeof(int));
			index++;
		}
		ifstr.close();
	...
	int ProcessData::operator()(std::string target, double& average, double& variance) {
		...
		ofstr.write((const char*)&total_items, sizeof(int));
		for (int i = 0; i < total_items; i++) {
			ofstr.write((const char*)&data[i], sizeof(int));
		}
		return 0;
	}
